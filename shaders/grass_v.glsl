...
void main()
    ...
    if(dot(blend_mask.rgb, vec3(1.0, 1.0, 1.0)) < 0.1)        
        ...
        float pc_impact=clamp(pow(1.0-distance(v.xy+pos.xy, pc_pos.xy)*0.1, 4.0)*anim_co, 0.0, 1.0);       
        float animation =sin(0.7*osg_FrameTime+float(gl_InstanceID))*sin(1.7*osg_FrameTime+float(gl_InstanceID))*anim_co;
       
        v.xy += animation*(1.0-pc_impact);
        v.x += 5.0 * pc_impact*sign(v.x+pos.x-pc_pos.x);
        v.y += 5.0 * pc_impact*sign(v.y+pos.y-pc_pos.y);
        v.z-=pc_impact*10.0;

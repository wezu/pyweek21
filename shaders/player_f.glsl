...
void main()
    ...
    //skin color bonus    
    color+=vec3(0.045, 0.025, 0.035)*skin; 
    ...
    for (int i=0; i<num_lights; ++i)
        ...
        //specular
        R=reflect(L,N)*att;
        specular +=pow(max(dot(R, V), 0.0), 12.0)*light_color[i].a*gloss*(1.0-skin);
        specular +=pow(max(dot(R, V), 0.0), 30.0)*light_color[i].a*gloss*skin;
        }
